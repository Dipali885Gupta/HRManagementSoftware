...
} from '../../../proxy/payroll-adjustments/models';
import { PayrollAdjustmentService } from '../../../proxy/payroll-adjustments/payroll-adjustment.service';

export abstract class AbstractPayrollAdjustmentViewService {
  protected readonly proxyService = inject(PayrollAdjustmentService);
  ...
  data: PagedResultDto<PayrollAdjustmentWithNavigationPropertiesDto> = {
    items: [],
    totalCount: 0,
  };

  selectionType = SelectionType;
  selected = signal<PayrollAdjustmentWithNavigationPropertiesDto[]>([]);
  allSelected = signal(false);
  selectedCount = computed(() => this.selected().length);

  filters = {} as GetPayrollAdjustmentsInput;
  ...
  protected bulkDeleteRequest() {
    const ids = this.selected().map(({ payrollAdjustment: { id } }) => id);
  ...
  delete(record: PayrollAdjustmentWithNavigationPropertiesDto) {
    this.confirmationService
      ...
      .pipe(
        ...
        switchMap(() => this.proxyService.delete(record.payrollAdjustment.id)),
      )
  ...
  hookToQuery() {
    ...
    const setData = (list: PagedResultDto<PayrollAdjustmentWithNavigationPropertiesDto>) => {
      this.data = list;
  ...
  clearFilters() {
    this.filters = {} as GetPayrollAdjustmentsInput;
    this.list.get();
  }
  ...
  exportToExcel() {
    ...
    this.proxyService
      ...
      .subscribe(result => {
        this.abpWindowService.downloadBlob(result, 'PayrollAdjustment.xlsx');
      });
  }
}
