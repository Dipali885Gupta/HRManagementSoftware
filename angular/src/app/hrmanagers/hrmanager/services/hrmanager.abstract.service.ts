...
} from '../../../proxy/hrmanagers/models';
import { HRManagerService } from '../../../proxy/hrmanagers/hrmanager.service';

export abstract class AbstractHRManagerViewService {
  protected readonly proxyService = inject(HRManagerService);
  ...
  data: PagedResultDto<HRManagerWithNavigationPropertiesDto> = {
    items: [],
    totalCount: 0,
  };

  selectionType = SelectionType;
  selected = signal<HRManagerWithNavigationPropertiesDto[]>([]);
  allSelected = signal(false);
  selectedCount = computed(() => this.selected().length);

  filters = {} as GetHRManagersInput;
  ...
  const ids = this.selected().map(({ hrManager: { id } }) => id);
  ...
  delete(record: HRManagerWithNavigationPropertiesDto) {
    this.confirmationService
      ...
      .pipe(
        ...
        switchMap(() => this.proxyService.delete(record.hrManager.id)),
      )
  ...
  hookToQuery() {
    ...
    const setData = (list: PagedResultDto<HRManagerWithNavigationPropertiesDto>) => {
      this.data = list;
  ...
  clearFilters() {
    this.filters = {} as GetHRManagersInput;
    this.list.get();
  }
  ...
  exportToExcel() {
    ...
    this.proxyService
      ...
      .subscribe(result => {
        this.abpWindowService.downloadBlob(result, 'HRManager.xlsx');
      });
  }
}
