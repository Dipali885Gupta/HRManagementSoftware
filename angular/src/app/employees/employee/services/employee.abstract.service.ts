...
export abstract class AbstractEmployeeViewService {
  protected readonly proxyService = inject(EmployeeService);
  ...
  data: PagedResultDto<EmployeeWithNavigationPropertiesDto> = {
    items: [],
    totalCount: 0,
  };

  selectionType = SelectionType;
  selected = signal<EmployeeWithNavigationPropertiesDto[]>([]);
  ...
  protected bulkDeleteRequest() {
    const ids = this.selected().map(({ employee: { id } }) => id);
  ...
  delete(record: EmployeeWithNavigationPropertiesDto) {
    this.confirmationService
      ...
      .pipe(
        ...
        switchMap(() => this.proxyService.delete(record.employee.id)),
      )
  ...
  hookToQuery() {
    ...
    const setData = (list: PagedResultDto<EmployeeWithNavigationPropertiesDto>) => {
      this.data = list;
  ...
  exportToExcel() {
    ...
    this.proxyService
      ...
      .subscribe(result => {
        this.abpWindowService.downloadBlob(result, 'Employee.xlsx');
      });
  }
}
