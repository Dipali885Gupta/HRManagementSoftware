import type { GetPayrollAdjustmentsInput, PayrollAdjustmentCreateDto, PayrollAdjustmentDto, PayrollAdjustmentExcelDownloadDto, PayrollAdjustmentUpdateDto, PayrollAdjustmentWithNavigationPropertiesDto } from './models';
...
export class PayrollAdjustmentService {
  ...
  deleteAll = (input: GetPayrollAdjustmentsInput, config?: Partial<Rest.Config>) =>
    this.restService.request<any, void>({
      ...
      params: { filterText: input.filterText, monthMin: input.monthMin, monthMax: input.monthMax, yearMin: input.yearMin, yearMax: input.yearMax, status: input.status, netpayMin: input.netpayMin, netpayMax: input.netpayMax, leaveRequestId: input.leaveRequestId, employeeId: input.employeeId, sorting: input.sorting, skipCount: input.skipCount, maxResultCount: input.maxResultCount },
    },
    { apiName: this.apiName,...config });
  ...
  getListAsExcelFile = (input: PayrollAdjustmentExcelDownloadDto, config?: Partial<Rest.Config>) =>
    this.restService.request<any, Blob>({
      ...
      params: { downloadToken: input.downloadToken, filterText: input.filterText, monthMin: input.monthMin, monthMax: input.monthMax, yearMin: input.yearMin, yearMax: input.yearMax, status: input.status, netpayMin: input.netpayMin, netpayMax: input.netpayMax, leaveRequestId: input.leaveRequestId, employeeId: input.employeeId },
    },
    { apiName: this.apiName,...config });
