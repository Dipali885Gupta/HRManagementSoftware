...
} from '../../../proxy/leave-requests/models';
import { LeaveRequestService } from '../../../proxy/leave-requests/leave-request.service';

export abstract class AbstractLeaveRequestViewService {
  protected readonly proxyService = inject(LeaveRequestService);
  ...
  data: PagedResultDto<LeaveRequestWithNavigationPropertiesDto> = {
    items: [],
    totalCount: 0,
  };

  selectionType = SelectionType;
  selected = signal<LeaveRequestWithNavigationPropertiesDto[]>([]);
  allSelected = signal(false);
  selectedCount = computed(() => this.selected().length);

  filters = {} as GetLeaveRequestsInput;
  ...
  protected bulkDeleteRequest() {
    const ids = this.selected().map(({ leaveRequest: { id } }) => id);
  ...
  delete(record: LeaveRequestWithNavigationPropertiesDto) {
    this.confirmationService
      ...
      .pipe(
        ...
        switchMap(() => this.proxyService.delete(record.leaveRequest.id)),
      )
  ...
  hookToQuery() {
    ...
    const setData = (list: PagedResultDto<LeaveRequestWithNavigationPropertiesDto>) => {
      this.data = list;
  ...
  clearFilters() {
    this.filters = {} as GetLeaveRequestsInput;
    this.list.get();
  }
  ...
  exportToExcel() {
    ...
    this.proxyService
      ...
      .subscribe(result => {
        this.abpWindowService.downloadBlob(result, 'LeaveRequest.xlsx');
      });
  }
}
